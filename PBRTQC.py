...
# 'Analytes Indices.xlsx' dosyasını okuma
file_path = 'Analytes Indices.xlsx'  # Dosya yolunu burada güncelleyin
df = pd.read_excel(file_path)
# DataFrame'i sözlüğe dönüştürme
...
def process_data_file(patient_category, measurement_lower_limit, measurement_upper_limit,  
    ...
    if 'Gender' and 'Age' and 'Sampling Time' and 'Results'  in df.columns:
        ...
        # Eğer "Servis Adı" sütunu yoksa, "+" işaretleme işlemini yapmayacağız
    ...
    # En erken ve en geç tarihleri al
    min_date = patient_filtered_df['Sampling Time'].min()
    max_date = patient_filtered_df['Sampling Time'].max()

    # Toplam gün sayısını hesapla (min ve max tarih arasındaki fark + 1 gün)
    total_days = (max_date - min_date).days + 1

    # Kaç hafta olduğunu bul ve bu haftaları 5 gün üzerinden hesapla
    ...
    # Veriyi ikiye bölün: İlk yarı ve ikinci yarı
    first_half_df = patient_filtered_df.iloc[:total_rows // 2]
    second_half_df = patient_filtered_df.iloc[total_rows // 2:]

    # İlk yarı için Male ve Female oranlarını hesaplayın
    ...
    # İkinci yarı için Male ve Female oranlarını hesaplayın
    ...
    if selected_analyte == "AST":
        patient_filtered_df["Results"] = patient_filtered_df["Results"] * 0.017
    elif selected_analyte == "Iron":
...
class MainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        # Analytes_List.keys() alfabetik olarak sıralanır
        sorted_analytes = sorted(Analytes_List.keys())

        # QComboBox oluşturulur ve sıralanmış liste eklenir
        ...
        mainLayout.addWidget(groupBox_training_data, 1)  # 1/3 alan kaplasın

        # Right side layout for control and performance
        rightLayout = QVBoxLayout()

        # Control Limits Determination Group Box
        ...
        self.patient_per_day_widget.valueChanged.connect(self.list_day_and_data)
        control_layout_canvas.addLayout(self.add_label_to_widget(self.patient_per_day_widget, "Patient Per Day"))
        ...
        rightLayout.addWidget(control_limit_data, 1)  # Sağ tarafta yarı alan kaplasın
        ...
        self.biased_point_widget = QComboBox()
        self.biased_point_widget.addItems(["100", "150", "200", "Random"])
        performance_layout_canvas.addLayout(self.add_label_to_widget(self.biased_point_widget, "Biased Point"))
        ...
        performance_layout_canvas.addLayout(self.add_label_to_widget(self.control_limit_type_widget, "Select Control Limit"))



        self.selected_benchmarking_widget = QComboBox()
        self.selected_benchmarking_widget.addItems(["Total AUC", "Total Allowable Area", "Total Non-Allowable Area", "Lowest Total AUC (*Bias)", "Total Allowable Area (*Bias)", "Total Non-Allowable Area (*Bias)", 
        "Sum of Min-Max Scale",  "Sum of Robust Scale",  "False Positive Rate", "MNPed", "MNPed (*Bias)"])
        performance_layout_canvas.addLayout(self.add_label_to_widget(self.selected_benchmarking_widget, "Select Benchmarking"))

        self.create_MNPed_button_widget = QPushButton("Create NPed Graphics")
        ...
        self.batch_size_widget = QSpinBox()
        self.batch_size_widget.setMaximum(1000000)
        self.batch_size_widget.setValue(1000)
        performance_layout_canvas.addLayout(self.add_label_to_widget(self.batch_size_widget, "Batch Size"))

        self.batch_monitoring_button = QPushButton("Batch Monitoring")
        self.batch_monitoring_button.clicked.connect(self.batch_monitoring)
        performance_layout_canvas.addLayout(self.add_label_to_widget(self.batch_monitoring_button, ""))

        performance_layout_canvas.addWidget(self.performance_canvas)
        performance_limit_data.setLayout(performance_layout_canvas)
        rightLayout.addWidget(performance_limit_data, 1)  # Sağ tarafta yarı alan kaplasın

        rightContainer = QWidget()
        rightContainer.setLayout(rightLayout)
        mainLayout.addWidget(rightContainer, 2)  # Sağ tarafın tamamı 2/3 oranında alan kaplasın
    ...
    def pick_automatic_PBRTQC(self):
        ...
        auto_process = automatic_PBRTQC(self.patient_category_widget.currentText(),
                                        ...
                                        self.patient_per_day_widget.value(), self.biased_point_widget.currentText(), 
    ...
    def select_csv(self):
        ...
        self.df,  self.total_patients, _, _, self.training_data_m_f_ratio, self.test_data_m_f_ratio, self.training_data_age_median, self.test_data_age_median  = process_data_file(self.patient_category_widget.currentText(),                                                                                                                                                                                self.measurement_lower_limit,self.measurement_upper_limit,
        gender_column_name, age_column_name, sampling_time_column_name, clinics_column_name,
        icu_name_list, inpatient_name_list, emergency_name_list, "Results", self.selected_analyte)

            
        self.patient_per_day = 1000 
        self.total_days = self.total_patients // self.patient_per_day
        # Günlük hasta sayısını widget'a ayarlama
        self.patient_per_day_widget.setValue(int(self.patient_per_day))
    ...
    def list_day_and_data(self):
        # DataFrame'in uzunluğunu al
        self.first_half_series, self.second_half_series = df_to_list(self.df, 0.5, 0.5, self.patient_per_day, return_type='pandas', exclude_last_list=True)
    ...
    def update_training_data_plot(self):
        ...
        if self.first_half_series_copy is not None and self.second_half_series_copy is not None:
            ...
            # İlk yarı üzerinde trunkasyon ve transformasyon işlemleri
            truncation_of_first_half = Truncation(
                self.flattened_training_data,  # Tüm seriyi trunkasyona tabi tutuyoruz
            ...
            # İkinci yarı üzerinde trunkasyon ve transformasyon işlemleri
            truncation_of_second_half = Truncation(
                self.flattened_test_data,  # Tüm seriyi trunkasyona tabi tutuyoruz
                self.truncation_parameter_widget.currentText(), 
                self.exclusion_parameter_widget.currentText(), 
                self.selected_limit_RCVg)
            ...
            # İkinci yarı için RNS hesaplayın
            ...
            #self.transformed_series_training_data_without_nan = remove_nan_from_lists(self.transformed_daily_series_first, "Transformed Training Data Series cleaning NaN")
            #self.transformed_series_test_data_without_nan = remove_nan_from_lists(self.transformed_daily_series_second, "Transformed Test Data Series cleaning NaN")
             
            #self.transformed_daily_series_first = convert_list_to_array(self.transformed_series_training_data_without_nan.copy(),"Transformed Training Data Array")
            #self.transformed_daily_series_second = convert_list_to_array(self.transformed_series_test_data_without_nan.copy(), "Transformed Test Data Array")         
    ...
    def draw_NPed_plot(self):
        ...
        performance_of_last_half_data = PBRTQC_Biased(len(self.flattened_test_data), self.second_half_series, self.patient_category_widget.currentText(),
                                                      ...
                                                      self.biased_point_widget.currentText(), self.patient_per_day_widget.value(), 
        ...
        self.performance_canvas.draw_MNPed_Plot(self.data_for_plot_manuel, false_positive_rate, sum_of_MNPed,
                                                ...
                                                self.biased_point_widget.currentText(),)
    ...
    def draw_performance_plot(self):
        ...
        all_benchmarking = [ "Lowest Sum of MNPed","Lowest Total AUC (*Bias)"] #"Highest Sum of ROC AUC", "Lowest Total AUC (*Bias)", "Robust CV MAD",
        '''all_benchmarking = ["Highest Sum of ROC AUC",
                            "Highest Sum of Accuracy",
                            "Robust CV MAD",
                            "Lowest Total AUC (*Bias)",
                            "Lowest Sum of MNPed",
                            "FPR (Bias = 0)",           
                            "Sum of arranged MNPed",
                            "MNPed / Total Day",
                            "MNPed * FPR",                           
                            "MNPed/n",
                            "MRNPed*FPR",] '''
        
        performance_metric = "MNPed" #"Lowest Total AUC (*Bias)" #, "Highest Sum of Accuracy", "Highest Sum of Sensitivity", "Highest Sum of ROC AUC",  
    ...
    def batch_monitoring(self):
        if self.truncation_parameter_widget.currentText() == 'No Truncation':
            self.lower_trun_limit = self.daily_min_5_percentile_limit 
            self.upper_trun_limit = self.daily_max_95_percentile_limit 
            
        self.performance_canvas.execute_batch_monitoring(self.batch_size_widget.value(), float(self.convert_factor_widget.text()), 
                                                         self.lambda_value, self.z_score, 
                                                         self.select_analyte_widget.currentText(), 
                                                         self.lower_trun_limit, self.upper_trun_limit, 
                                                         self.daily_min_5_percentile_limit , self.daily_max_95_percentile_limit ,  
                                                         self.transformation_parameter_widget.currentText(),
                                                         self.statistical_parameter_widget.currentText(),
                                                         self.block_size_widget.value(),)
...
class Truncation:
    ...
    def truncate_data(self, trun_lower_limit, trun_upper_limit, block_size, n_sigmas):
        ...
        # NaN değerlerinin indekslerini sakla
        nan_indices = np.isnan(truncated_data)
        
        if self.truncation_param == "No Truncation":
            return truncated_data
        elif self.truncation_param == "Hampel":
            result = hampel(truncated_data, block_size, float(n_sigmas))
            if self.exclusion_param == "Winsorization":
                truncated_data = np.array(result.filtered_data)
            elif self.exclusion_param == "Trimming":
                outlier_indices = result.outlier_indices
                truncated_data[outlier_indices] = np.nan
        else:
        ...
        # Orijinal NaN indekslerini koru
        truncated_data[nan_indices] = np.nan
        
        # Truncasyon sonrası NaN olmayan veri kontrolü
        if np.all(np.isnan(truncated_data)):
            print("Uyarı: Trunkasyon sonrası tüm veriler NaN oldu.")
...
class Transformation:
    ...
    def transform_data(self):
        # NaN değerlerini kontrol etmek için np.isnan kullanılır
        data_array = np.array(self.data)
        nan_indices = np.isnan(data_array)
        
        # NaN değerleri çıkarılmış veri (NumPy Array kullanıyoruz)
        ...
        # Transformasyon yapılmış veriyi orijinal veri boyutunda oluştur
...
class StatisticalProcess:
    def __init__(self, data, statistical_param, block_size, z_score, alpha_value=None, keep_nan_index=True):
        self.data = data  # Pandas Series: Her bir element bir numpy array
        self.statistical_param = statistical_param
        self.block_size = block_size
        self.alpha_value = alpha_value
        self.keep_nan_index = keep_nan_index
  
    def moving_mad_fast(self, data, window):
        mad_values = np.full(len(data), np.nan)
        for i in range(window - 1, len(data)):
            window_data = data[i - window + 1:i + 1]
            median = np.median(window_data)
            mad_values[i] = np.median(np.abs(window_data - median))
        return mad_values

    def harrell_davis_percentile_estimator(self, x):
        return hdquantiles(x, prob=[0.5])[0]

    def alternative_moving_average(self, x, alpha):
        xb = np.zeros_like(x)
        xb[0] = x[0]
        for i in range(1, len(x)):
            deviation = np.sign(x[i] - xb[i - 1]) * np.abs(x[i] - xb[i - 1])
            new_value = (2 - alpha) * xb[i - 1] + alpha * deviation
    ...
    def remove_nan_and_process(self, array_data):
        # NaN değerlerinin indekslerini bul
        nan_indices = np.isnan(array_data)
        # NaN olmayan veriyi al
        ...
        elif self.statistical_param == "HD50":
            processed_data_no_nan = pd.Series(data_no_nan).rolling(window=self.block_size).apply(self.harrell_davis_percentile_estimator).values
        elif self.statistical_param == "Moving MAD":
            processed_data_no_nan = self.moving_mad_fast(data_no_nan, self.block_size)
        ...
        elif self.statistical_param == "Bulls Algorithm (XB)":
            processed_data_no_nan = self.alternative_moving_average(data_no_nan, self.alpha_value)
        else:
            raise ValueError(f"Unknown statistical parameter: {self.statistical_param}")
        
        # Eğer self.keep_nan_index True ise NaN değerlerini eski pozisyonlarına yerleştir
...
class ControlLimitDetermination:
    ...
    def calculate_control_limits(self):         
        # Günlük veri istatistiklerini hesaplama
        daily_data_list = []
        ...
        # Verileri alt, orta ve üst üçte birlik dilimlere ayırma
        ...
        # Limitleri aşan günlük değerlerin sayısını hesapla
        ...
        # Exceedances yüzdelerini hesaplayın
    ...
    def optimize_limits(self,initial_lower_limit, initial_upper_limit):
        # Mevcut verileri optimize ederek yeni alt ve üst limitleri tahmin et
            ...
            for daily_data in self.daily_statistical_data_series:
                for value in daily_data:
                    if not (lower_limit <= value <= upper_limit):
                        false_day_count += 1
                        break  # Bu gün limitlerin dışında, diğer güne geç
            # Hedef %10'luk false day count'u yakalamak
            false_day_percentage = (false_day_count / total_day_count) * 100
            return abs(false_day_percentage - 10)  # %10'a ne kadar yakınsa o kadar iyi


        initial_limits = [initial_lower_limit, initial_upper_limit]

        # Sınırları optimize et
        result = minimize(objective, initial_limits, method='Nelder-Mead')
        optimal_limits = result.x

        # Yeni alt ve üst limitleri döndür
...
class PBRTQC_Biased:
    def __init__(self, whole_total_patient, data, patient_category, training_data_m_f_ratio, test_data_m_f_ratio, training_data_age_median, test_data_age_median, training_truncation_lower, training_truncation_upper, 
        ...
        if bias_adding_point != "Random":
            self.bias_adding_point = int(bias_adding_point)
        else:
            self.bias_adding_point = bias_adding_point
    ...
    def counting_saving_NPed(self):
        ...
        for bias_category in BIAS_CATEGORIES:
            ...
            nped_count_ratio = []
            daily_count = 0
            if self.bias_adding_point != "Random": 
                day_number = 0
            ...
            if bias_category == 0:
                ...
                # False Day Count sayımı
                for x_daily_data in transformed_series_test_data_without_nan:
                    # Günlük veri listesinin max ve min değerlerini al
                    max_value = max(x_daily_data)
                    min_value = min(x_daily_data)

                    # Eğer max self.upper_control_limit'ten büyükse
                    if max_value > self.upper_control_limit:
                        false_day_count += 1  # Yanlış gün sayısını artır

                    # Eğer min self.lower_control_limit'ten küçükse
                    if min_value < self.lower_control_limit:
                        false_day_count += 1  # Yanlış gün sayısını tekrar artır

                # FPR yüzdesini hesapla
            ...
            # Eğer bias uygulanmış günler varsa ortalama, medyan, min ve max değerlerini hesapla
        ...
        self.columns['AUC (*Bias)'] = area_NPed_multiplied
        ...
        # Tüm kolonların aynı uzunlukta olmasını sağlamak için kolonları dengeleyin
    ...
    def counting_saving_NPed_manuel(self,selected_analyte):   
            ...
            print(f'Kaydedilen dosya : {performance_file_name}')
...
class Plot_Training_Data(FigureCanvas):
    ...
    def training_and_test_data_plot(self, analyte, training_data_with_nan, test_data_with_nan, transformation_parameter, 
        ...
        # Eğitim verileri için istatistiksel değerler ve boxplot
...
class Plot_Control_Limit(FigureCanvas):
    ...
    def control_limit_plot(self, daily_statistical_data_series, daily_data_parameters_list, 
        ...
        # Günlük verilerin gün numarasına göre x değerlerini oluşturma
        x_values = np.arange(new_total_days)
                          
        try:
            for i, daily_data in enumerate(daily_statistical_data_series_without_nan):
                # Ortalama ve %5-%95 persentil değerlerini çizme (errorbar)
            ...
            # Seçili limit kalın çizilecek
            line_properties = {'linewidth': 3}  # Kalınlık için

            # Daily min max %5 ve %95 persentile limits
            ...
            # Entire Data 0.25 ve 99.75 limitleri
...
class PlotPatientErrorDetection(FigureCanvas):
    ...
    def draw_MNPed_Plot(self, data_for_plot, false_positive_rate, sum_of_MNPed, 
        ...
        self.clear_axes()  # Önce grafikleri temizleyelim

        # Verilerin depolanacağı listeler
        NPed_values, min_values, max_values, bias_values = [], [], [], []

        # Bias kategorilerini işleyelim
        ...
        # Verileri flatten (düzleştir) edelim
...
class automatic_PBRTQC:
    def __init__(self,  patient_category, selected_analyte, date_column_name, process_column_name, 
                 ...
                 patient_per_day, biased_adding_point, convert_factor, control_limit_type ):
        ...
        self.patient_per_day = 1000
        self.ratio = 0.5
        if biased_adding_point != "Random":
            self.biased_adding_point = 100
        else:
            self.biased_adding_point = 100
        self.convert_factor = convert_factor
        self.plot_for_data_folder_name = f'Performances/{self.selected_analyte}_{self.patient_per_day}_{self.biased_adding_point}'
    ...
    def select_csv(self):
                ...
                self.df, self.total_patients, _, _, self.training_data_m_f_ratio, self.test_data_m_f_ratio, self.training_data_age_median, self.test_data_age_median, = process_data_file( self.patient_category,self.measurement_lower_limit,self.measurement_upper_limit,
                    gender_column_name, age_column_name, sampling_time_column_name, clinics_column_name,
                    icu_name_list, inpatient_name_list, emergency_name_list, "Results",  self.selected_analyte)
    ...
    def list_day_and_data(self):
        # DataFrame'in uzunluğunu al
        self.first_half_series, self.second_half_series = df_to_list(self.df, self.ratio, 1 - self.ratio, self.patient_per_day, return_type='pandas', exclude_last_list=True)
        ...
        try:
            ...
            for block_size in BLOCK_SIZE:
                for truncation_parameter in TRUNCATION_CATEGORIES:
                    # truncation_parameter "No Truncation" ise tüm transformation kategorilerini kullan
    ...
    def merge_batch_files(self, selected_analyte, patient_per_day, patient_category):
        ...
        if not batch_files:
            print(f"Uyarı: '{self.plot_for_data_folder_name}' klasöründe birleştirilecek dosya bulunamadı.")
            return
        ...
        print(f'Bütün batch dosyaları {combined_file_name} olarak birleştirildi.')
...
class GroupedDataPlotter:
    ...
    def calculate_grouped_data(self, df, control_limit_type, max_allowable_bias,):
        ...
        for index, row in df.iterrows():
            ...
            sum_of_accuracy_median = sum(x for x in accuracy_values if not math.isnan(x))
            ...
            allowable_area_NPed = np.trapz(
                [p for i, p in enumerate(area_NPed_bias) if not np.isnan(p) and abs(bias_values_for_trapz[i]) <= max_allowable_bias],
                x=[b for i, b in enumerate(bias_values_for_trapz) if not np.isnan(area_NPed_bias[i]) and abs(b) <= max_allowable_bias])
            non_allowable_area_MNPed = area_NPed - allowable_area_NPed
            ...
            if sum_of_fpr <= 5:
                ...
                results.append({
                    ...
                    'ROC_points': roc_values,
                    'accuracy_points': accuracy_values,
                    'sensitivity_points': sensitivity_values,
                    'area_NPed': area_NPed,
                    'allowable_area_NPed': allowable_area_NPed,
                    'non_allowable_area_MNPed': non_allowable_area_MNPed,
                    'area_NPed_multiplied': area_NPed_multiplied,
                    'fpr':sum_of_fpr,
                    'sum_of_accuracy_medians': sum_of_accuracy_median,
        ...
        benchmark = []
        if self.selected_benchmarking == "Lowest Total AUC (*Bias)":
        ...
        try:
            ...
            for j in range(0, length_of_process,self.max_sorted_number):
                ...
                columns = ["Analyte", "Patient Per Day", "Bias Point", "CVi", "CVg", "II", "TEa", "RCVg",  "Performance Order", "Patient Group Included", "Truncation (Exclusion)",  "Transformation", "Moving Statistics", "Block Size",
                           "Data Size after Truncation", "Robust CV MAD", "FPR",  "Sum of MNPed", "Sum of Total AUC (*Bias)",  ]
               
                if self.performance_metric == "MNPed":
                    comparision_metric = 'NPed_points'
                elif self.performance_metric == "Lowest Total AUC (*Bias)":
                ...
                for i in range(self.max_sorted_number - 1, -1, -1): 
                    if i < len(sorted_benchmarking):
                        ...
                        if sorted_benchmarking[i][comparision_metric]:  # Eğer liste boş değilse çizim yap
                            filtered_bias_categories = []
                            filtered_NPed_points = []

                            # Bias kategorileriyle ilgili her bir noktayı kontrol ediyoruz
                            for b, v in zip(BIAS_CATEGORIES, sorted_benchmarking[i][comparision_metric]):
                                # Bias -1 ve geçerli v varsa
                                if b == -1 and not np.isnan(v):
                                    # -1 değerini ekle
                                    filtered_bias_categories.append(b)
                                    filtered_NPed_points.append(v)

                                    # -1’den sonra sanal noktalar ekle, sonsuza doğru gitsin
                                    virtual_bias_points = np.linspace(-0.9, -0.01, 10)  # -0.9 ile -0.01 arasında 10 sanal nokta
                                    virtual_y_values = (10**4) * -1 / virtual_bias_points  # Sonsuza doğru giden negatif değerler
                                ...
                                # Bias 1 ve geçerli v varsa
                                    ...
                                    # 1’den sonra sanal noktalar ekle, sonsuza doğru gitsin
                                    virtual_bias_points = np.linspace(0.01, 0.9, 10)  # 0.01 ile 0.9 arasında 10 sanal nokta
                                    virtual_y_values = (10**4) / virtual_bias_points  # Sonsuza doğru giden pozitif değerler
                                    filtered_bias_categories.extend(virtual_bias_points)
                                    filtered_NPed_points.extend(virtual_y_values)

                                # Normal değerler için, sonsuz olmayan değerleri filtrele
                                elif not np.isnan(v) and not np.isinf(v):
                                    filtered_bias_categories.append(b)
                                    filtered_NPed_points.append(v)


                            if filtered_NPed_points:  # Eğer çizilecek değer kaldıysa
                                line2, = ax_main.plot(
                                ...
                                "Sum of Total AUC (*Bias)": [int(sorted_benchmarking[i]['area_NPed_multiplied'])],
                                "FPR": [sorted_benchmarking[i]['fpr']],                                  
                  
                                })
                                results_df = pd.concat([results_df, temp_df], ignore_index=True)  
                            
                                if i == 0:  # Sadece kesikli çizgi için
                                    for j in range(1, len(filtered_bias_categories)):
                                        ...
                                        # -max_allowable_bias ile kesişim kontrolü
                                        if x1 < -max_allowable_bias < x2 or x1 > -max_allowable_bias > x2:
                                            ...
                                            # Kesim noktası çizimi (-max_allowable_bias)
                ...
                # Sıralanmış DataFrame'i Excel dosyasına kaydetme
                sorted_df.to_excel(save_path_table, index=False)
            
                # Apply gradient and imshow
                ...
                # X ekseni üzerinde görünen değerler
                xticks = [int(b) for b in BIAS_CATEGORIES if int(b) % 10 == 0]
                # -max_allowable_bias ve +max_allowable_bias değerlerini ekle
                xticks.append(-max_allowable_bias)
                xticks.append(max_allowable_bias)

                # Etiketler için aynı işlemi yapalım ve her birine "%" ekleyelim
                xticklabels = [f"{b}%" for b in BIAS_CATEGORIES if int(b) % 10 == 0]
                # Bu değerlere karşılık gelen etiketlere de "%" ekleyelim
                xticklabels.append(f"-{max_allowable_bias:.1f}%")
                xticklabels.append(f"{max_allowable_bias:.1f}%")

                # X ekseni için tikleri ve etiketleri ayarla
                ax_main.set_xticks(xticks)
                ax_main.set_xticklabels(xticklabels, rotation=45)
                ...
                # Set Y-axis limit based on the maximum value of non-NaN points
                y_max_value = sorted(
                                    [v for v in sorted_benchmarking[0][comparision_metric] if not np.isnan(v) and not np.isinf(v)],
                                        reverse=True
                                        )[1]  # 2. büyük değeri almak için
                ...
                # Kalın metin ayarı (legend'deki belirli bir etiketi kalın yapmak)
                ...
                file_name = f'{selected_analyte}_{label_selected_benchmarking}_group_{j // 5 + 1}.tiff'
...
# Listeyi tek bir array'e dönüştürme fonksiyonu (her elemanı düzleştirerek)
def convert_list_to_array(series, data_title=None):
    # Series'in her elemanını birleştirip tek boyutlu bir NumPy array oluşturuyoruz
...
def convert_array_to_list(array, original_shapes, data_type, data_title=None):
    ...
    if data_type == "python_list":
        for shape in original_shapes:
            new_list.append(array[index:index + shape].tolist())  # Listeye çeviriyoruz
            index += shape
    else:
        for shape in original_shapes:
            new_list.append(array[index:index + shape])  # NumPy array olarak ekliyoruz
            index += shape
    if data_title :        
...
# Her bir listeden NaN değerlerini kaldıran fonksiyon
def remove_nan_from_lists(series, data_title=None, from_index=None):
    def clean_list(x):
        if isinstance(x, list):
            # Tüm NaN'ları temizlemeden önce orijinal listedeki sıralı indeksleri tut
            if from_index is not None and from_index < len(x):
                original_index = from_index  # from_index'li değerin indeksini al
            else:
                original_index = None
            ...
            if original_index is not None and original_index < len(x):
                # NaN'lar temizlendikten sonra orijinal indekse karşılık gelen değerin yeni indeksini bul
                new_index = len([i for i in x[:original_index] if not pd.isna(i)])  # from_index'li değerin yeni indeksini bul
            ...
            if original_index is not None and original_index < len(x):
                # NaN'lar temizlendikten sonra orijinal indekse karşılık gelen değerin yeni indeksini bul
                new_index = len(x[:original_index][~pd.isna(x[:original_index])])  # Orijinal from_index'teki değerin yeni indeksi
    ...
    # Eğer data_title verilmişse, detayları göster
    if data_title:
...
def remove_nan_from_array(array, data_title=None):
    # NumPy array'deki NaN değerlerini kaldırma
    cleaned_array = array[~np.isnan(array)]  # NaN olmayan değerleri alır

    # İsteğe bağlı başlık ve bilgi yazdırma
    if data_title:
...
def df_to_list(data, first_part_ratio, second_part_ratio, data_per_day, return_type='pandas', exclude_last_list=True):
    ...
    def split_into_chunks(df, chunk_size, exclude_last):
        ...
        if exclude_last and len(chunked_data[-1]) < chunk_size:
            chunked_data = chunked_data[:-1]  # Son listeyi çıkar
        return chunked_data

    # İlk ve ikinci parçayı data_per_day uzunluğunda parçalara böl
    ...
    # İsteğe bağlı olarak pandas veya numpy formatında döndür
...
def add_bias(series, bias_adding_point, factor, random_mode=False):
    def apply_bias_to_list(data_list, start_point, end_point):
        # Belirtilen aralıktaki değerlere bias ekle
        for i in range(start_point, end_point):
            data_list[i] *= (1 + (factor / 100))
        return data_list

    # Eğer random_mode=False ise, belirtilen noktadan itibaren bias uygular
    if not random_mode:
        # Her bir listeye bias ekle
        return series.apply(lambda x: apply_bias_to_list(np.array(x), bias_adding_point, len(x)))
    
    # Eğer random_mode=True ise, rastgele bir aralıktan itibaren bias uygular
    else:
        def apply_random_bias(data_list):
            # Rastgele bir başlangıç ve bitiş noktası seç (en az 20 değer içerecek şekilde)
            if len(data_list) >= 20:
                start_point = random.randint(0, len(data_list) - 21)
                end_point = random.randint(start_point + 20, len(data_list))
                return apply_bias_to_list(np.array(data_list), start_point, end_point)
            else:
                return data_list  # Eğer veri 20'den azsa, bias uygulanmaz
       
        # Her bir listeye rastgele bias ekle
        return series.apply(lambda x: apply_random_bias(np.array(x)))      
